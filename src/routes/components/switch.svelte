<script lang="ts">
    ...
    const setPositionMutation = createResourceMutation(client, 'setPosition', () => positionQuery.queryKey);
...
{#if positionQuery.data !== undefined && numPositionsQuery.data !== undefined}
  <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-800/50">
    <p class="mb-3 text-sm font-medium text-neutral-600 dark:text-neutral-400">
      Current position: <span class="font-semibold text-neutral-900 dark:text-white">{numPositionsQuery.data[1][positionQuery.data]}</span>
    </p>
    <div class="flex flex-wrap gap-2">
      {#each numPositionsQuery.data[1] as label, index (index)}
        <button
          class="rounded-md border px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-neutral-400 focus:ring-offset-2 dark:focus:ring-neutral-500 dark:focus:ring-offset-neutral-900 {positionQuery.data === index
