...
Use `gh api` to fetch source directly from the Viam repos:
...
- Code uses a type or method from `@viamrobotics/sdk` and the usage looks incorrect — verify the actual exported API in the `viam-typescript-sdk` source repo.
- A hook is being written for a new resource type — fetch the corresponding `.proto` from `viamrobotics/api` to verify field names and RPC signatures.
